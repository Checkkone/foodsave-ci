...
export default function DashboardVendeur() {
  ...
  async function chargerDonnees() {
    ...
    if (offresError) {
      console.error("Erreur offres:", offresError);
      return;
    }
    setOffres(offresData || []);

    // Charger les réservations liées aux offres du vendeur
  ...
  async function publierOffre(e) {
    ...
    // Validation : prix réduit doit être inférieur au prix normal
  ...
  // Calcul des revenus réels basé sur les prix des offres réservées
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs text-gray-400">{offres.length} offre(s) publiée(s)</p>
            </div>
            ...
            {offres.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {offres.map(o => {
                  ...
                  return (
                    <div key={o.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className="ml-2 bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded-lg flex-shrink-0">
                          -{red}%
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-4">
                        <div>
                          ...
                          <span className="text-orange-500 font-extrabold text-xl ml-2">
            ...
            <form onSubmit={publierOffre} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  value={form.nom_plat}
                  onChange={e => setForm({ ...form, nom_plat: e.target.value })}
                  required />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setForm({ ...form, prix_normal: e.target.value })}
                    required />
                </div>
                <div>
              ...
              {reduction > 0 && reduction <= 100 && (
                <div className={`border rounded-xl p-3 text-center ${reduction > 70 ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-200"}`}>
                  <p className={`font-semibold text-sm ${reduction > 70 ? "text-green-700" : "text-orange-700"}`}>
                    🎉 Tu offres <strong>{reduction}%</strong> de réduction — très attractif pour les étudiants !
                  </p>
                </div>
              )}
              {reduction < 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-center">
                  <p className="text-red-600 font-semibold text-sm">
                    ⚠️ Le prix réduit doit être inférieur au prix normal
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-gray-600 mb-1 block">Quantité disponible *</label>
                  <input
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-400 transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setForm({ ...form, heure_limite: e.target.value })}
                    required />
                </div>
              </div>
          ...
          <div>
            {reservations.length === 0 ? (
              <div className="bg-white rounded-2xl p-16 text-center border border-gray-100 shadow-sm">
                ...
                <p className="text-gray-400 text-sm mt-1">Les réservations apparaîtront ici dès qu'un étudiant réserve</p>
              </div>
            ) : (
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-bold text-gray-800">
                            {offre?.nom_plat || "Plat inconnu"}
                          </p>
                          <p className="text-gray-400 text-sm mt-0.5">
                            Client : {r.utilisateurs?.nom || "Étudiant"}
                          </p>
